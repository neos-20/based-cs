...
def is_word_guessed(secret_word, letters_guessed):
    ...
    returns: boolean, True if all the letters of secret_word are in letters_guessed;
      False otherwise
    '''
    word = [i for i in secret_word]
    for i in word:
        for j in letters_guessed:
            if i == j:
                break
            elif j == letters_guessed[len(letters_guessed)-1]:
                return False
    return True
...
def get_available_letters(letters_guessed):
    ...
    all = [i for i in string.ascii_lowercase]
    for i in letters_guessed:
      if "".join(all).find(i) == -1:
        continue
      else:
        all.remove(i)
    return "".join(all)
...
def hangman(secret_word):
    ...
    * After each guess, you should display to the user the 
    ...
    while guesses > 0:
      ...
      inp = str.lower(input('Please guess a letter: '))
      if "".join(letters_guessed).find(inp) >= 0 or string.ascii_lowercase.find(inp) < 0:
        if warnings == 0:
          guesses -= 1
          warnings = 3
      ...
      if is_word_guessed(secret_word, letters_guessed) == True:
        ...
        print('Your total score for this game is: ' + str(len(set(secret_word))))
        return
    print('-----------')
